        ...
        def getSortedDomain(row, col):
            ...
            for i in range(3):
                for j in range(3):
                    r = blockR + i
                    c = blockC + j
                    if isUnfilledCell(r, c):
                        cells.add((r, c))

            for i in range(9):
                if isUnfilledCell(row, i):
                    cells.add((row, i))
                if isUnfilledCell(i, col):
                    cells.add((i, col))

            def sortingKey(num):
                constrainedNeighbourCounter = 0
                for (r, c) in cells:
                    if num in possibleValues[r][c]:
                        constrainedNeighbourCounter += 1
                return constrainedNeighbourCounter

            newDomain = list(possibleValues[row][col])
            if len(newDomain) > 0:
                sorted(newDomain, key=sortingKey)
            return newDomain

        currElement = getNextUnsetCell()
        currR, currC = currElement
        index = 0
        domain = getSortedDomain(currR, currC)
        isValidIndex = lambda: index < len(domain)

        # Main backtracking loop
        while isValidIndex() or len(setCells) > 0:
            if isValidIndex():
                currVal = domain[index]
                ...
                setCells.append((currR, currC, index, domain))
                ...
                currR, currC, index, domain = setCells.pop()
                currVal = domain[index]
